 ...
 * modify by jingws 2018-10-12  0.4.2.1
 */
...
(function(root, undefined) {
	...
	lib.settings = {
		currency : {
			...
			format : "%s%v", // controls output: %s = symbol, %v = value (can
			// be object, see docs)
	...
	/**
	 * Tests whether supplied parameter is an array from underscore.js,
	...
	function defaults(object, defs) {
		...
		for (key in defs) {
			if (defs.hasOwnProperty(key)) {
				// Replace values with defaults only if undefined (allow
				// empty/zero values):
				if (object[key] == null)
					object[key] = defs[key];
			}
	 ...
	 * value. Defers to native Array.map if available
	 */
	...
	function checkCurrencyFormat(format) {
		...
		if (isString(format) && format.match("%v")) {
			...
			// If no format, or object is missing valid positive value, use
			// defaults:
		} else if (!format || !format.pos || !format.pos.match("%v")) {

			// If defaults is a string, casts it to an object for faster
			// checking next time:
	...
	var unformat = lib.unformat = lib.parse = function(value, decimal) {
		...
		var regex = new RegExp("[^0-9-" + decimal + "]", [ "g" ]), unformatted = parseFloat(("" + value)
				.replace(/\((?=\d+)(.*)\)/, "-$1") // replace bracketed values
				// with negatives
	...
	var formatPercent = lib.formatPercent = function(number, needMulti,
			precision, nullPerSym) {
		if (!!!number) {
			if (nullPerSym)
				return '-%';
			return '-';
		...
		let val = formatNumber(number, precision);
		return val + '%';
	}
	/**
	 ...
	 * 2nd parameter `precision` can be an object matching `settings.number` add
	 * by jingws 20181012 param negativeSym ifval>0 add Symbol '+'
	 */
	var formatNumber = lib.formatNumber = lib.format = function(number,
			precision, negativeSym, thousand, decimal) {
		...
		// Do some calc:
		negative = number < 0 ? "-" : (negativeSym ? "+" : ""), base = parseInt(
				toFixed(Math.abs(number || 0), usePrecision), 10)
				+ "", mod = base.length > 3 ? base.length % 3 : 0;

		// Format the number:
		return negative
				+ (mod ? base.substr(0, mod) + opts.thousand : "")
				+ base.substr(mod).replace(/(\d{3})(?=\d)/g,
						"$1" + opts.thousand)
	 ...
	 * 
	 * Localise by overriding the symbol, precision, thousand / decimal
	 * separators and format Second param can be an object matching
	 * `settings.currency` which is the easiest way.
	 * 
	...
	/**
	 * Format a list of numbers into an accounting column, padding with
	 * whitespace to line up currency symbols, thousand separators and decimals
	 * places
	 * 
	 ...
	 * 
	 * NB: `white-space:pre` CSS rule is required on the list container to
	 * prevent browsers from collapsing the whitespace in the output strings.
	 */
	lib.formatColumn = function(list, symbol, precision, thousand, decimal,
		...
		// Check format (returns object with pos, neg and zero), only need pos
		// for now:
		formats = checkCurrencyFormat(opts.format),
		...
		formatted = map(list, function(val, i) {
			if (isArray(val)) {
				// Recursively format columns if list is a multi-dimensional
				// array:
				...
				// Format this value, push into formatted list and save the
				// length:
				fVal = useFormat.replace('%s', opts.symbol).replace(
						'%v',
						formatNumber(Math.abs(val),
								checkPrecision(opts.precision), opts.thousand,
								opts.decimal));
		...
		return map(formatted, function(val, i) {
			// Only if this is a string (not a nested array, which would have
			// already been padded):
	...
	// Export accounting for CommonJS. If being loaded as an AMD module, define
	// it as such.
	...
	if (typeof exports !== 'undefined') {
		...
		// Use accounting.noConflict to restore `accounting` back to its
		// original value.
